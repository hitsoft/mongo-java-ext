...
public class Managed {
    ...
    public static class QueryBuilder<T extends ManagedObject> {
        ...
        public QueryBuilder<T> or(Collection<QueryBuilder<T>> builders) {
            List<SearchBuilder> conditions = new ArrayList<>();
            for (QueryBuilder<T> builder : builders) {
                conditions.add(builder.searchBuilder);
            }
            searchBuilder.or(conditions);
        ...
        public class Exec<T extends ManagedObject> {
            ...
            public List<DBObjectExt> groupCount(Enum field) {
                return aggregate()
                        .group(Managed.Aggregate.Group.start()
                                ._id(field)
                                .count(ManagedObject.Field.TOTAL)
                        ).exec().fetch();
            }
        }
    }
